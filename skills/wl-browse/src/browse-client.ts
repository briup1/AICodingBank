 ...
 *   2. State file fallback: read `BROWSE_STATE_FILE` env or `<git-root>/.wl-ai-skills/browse/browse.json`
...
export interface BrowseClientOptions {
  ...
  timeoutMs?: number;
  /** Override state-file path. Default: BROWSE_STATE_FILE env or <git-root>/.wl-ai-skills/browse/browse.json. */
  stateFile?: string;
}
...
export function resolveBrowseAuth(opts: BrowseClientOptions = {}): ResolvedAuth {
  ...
  throw new Error(
    ...
    '(.wl-ai-skills/browse/browse.json must exist).'
  );
}
...
function defaultStateFile(): string | null {
  try {
    ...
    return path.join(base, '.wl-ai-skills', 'browse', 'browse.json');
  } catch {
    return path.join(process.cwd(), '.wl-ai-skills', 'browse', 'browse.json');
  }
}
