 ...
 *
 * State:
 *   State file: <project-root>/.wl-ai-skills/browse/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.wl-ai-skills/browse/browse-{console,network,dialog}.log
...
/**
 * Read ngrok authtoken from env var, ~/.local/state/wl-ai-skills/browse/ngrok.env, or ngrok's native
...
function resolveNgrokAuthtoken(): string | null {
  ...
  const ngrokEnvPath = path.join(home, '.local', 'state', 'wl-ai-skills', 'browse', 'ngrok.env');
...
function generateHelpText(): string {
  ...
  const lines = ['wl-browse — headless browser for AI agents', '', 'Commands:'];
...
async function shutdown(exitCode: number = 0) {
  ...
  const profileDir = path.join(process.env.HOME || '/tmp', '.local', 'state', 'wl-ai-skills', 'browse', 'chromium-profile');
...
async function start() {
  ...
  // except /connect.  Denials are logged to ~/.local/state/wl-ai-skills/browse/security/attempts.jsonl.
    ...
    if (url.pathname === '/connect' && req.method === 'GET') {
      ...
      if (url.pathname === '/welcome') {
        const welcomePath = (() => {
          ...
          // would resolve to ~/.local/state/wl-ai-skills/browse/projects/../../etc/passwd/... — path
          ...
          const projectWelcome = `${homeDir}/.local/state/wl-ai-skills/browse/projects/${slug}/designs/welcome-page-20260331/finalized.html`;
  ...
  if (process.env.BROWSE_TUNNEL === '1') {
    const authtoken = resolveNgrokAuthtoken();
    if (!authtoken) {
      console.error('[browse] BROWSE_TUNNEL=1 but no NGROK_AUTHTOKEN found. Set it via env var or ~/.local/state/wl-ai-skills/browse/ngrok.env');
    } else {
