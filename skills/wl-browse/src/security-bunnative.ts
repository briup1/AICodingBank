...
export function getCachedTokenizer(): TokenizerState {
  ...
  const dir = path.join(os.homedir(), '.local', 'state', 'wl-ai-skills', 'browse', 'models', 'testsavant-small');
...
export async function classify(text: string): Promise<ClassifyResult> {
  ...
  env.localModelPath = path.join(os.homedir(), '.local', 'state', 'wl-ai-skills', 'browse', 'models');
