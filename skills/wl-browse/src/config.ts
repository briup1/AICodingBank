 ...
 *   2. git rev-parse --show-toplevel → projectDir/.wl-ai-skills/browse/
...
export function resolveConfig(
  ...
  if (env.BROWSE_STATE_FILE) {
    ...
    projectDir = path.dirname(path.dirname(stateDir)); // parent of .wl-ai-skills/
  } else {
    projectDir = getGitRoot() || process.cwd();
    stateDir = path.join(projectDir, '.wl-ai-skills', 'browse');
...
/**
 * Create the .wl-ai-skills/browse/ state directory if it doesn't exist.
...
export function ensureStateDir(config: BrowseConfig): void {
  ...
  // Ensure .wl-ai-skills/ is in the project's .gitignore
  const gitignorePath = path.join(config.projectDir, '.gitignore');
  try {
    const content = fs.readFileSync(gitignorePath, 'utf-8');
    if (!content.match(/^\.wl-ai-skills\/?$/m)) {
      const separator = content.endsWith('\n') ? '' : '\n';
      fs.appendFileSync(gitignorePath, `${separator}.wl-ai-skills/\n`);
    }
  } catch (err: any) {
    if (err.code !== 'ENOENT') {
