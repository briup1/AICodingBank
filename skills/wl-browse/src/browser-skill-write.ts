 ...
 *
 * /skillify stages a candidate skill into ~/.local/state/wl-ai-skills/browse/.tmp/skillify-<spawnId>/,
...
export function stageSkill(opts: StageSkillOptions): string {
  ...
  const tmpRoot = opts.tmpRoot ?? path.join(os.homedir(), '.local', 'state', 'wl-ai-skills', 'browse', '.tmp');
