 ...
 *
 * Scope:
 *   - per-project: ~/.local/state/wl-ai-skills/browse/projects/<slug>/learnings.jsonl
 *   - global:      ~/.local/state/wl-ai-skills/browse/global-domain-skills.jsonl
 *
...
function gstackHome(): string {
  return process.env.WL_AI_SKILLS_STATE_HOME || path.join(os.homedir(), '.local', 'state', 'wl-ai-skills', 'browse');
}
