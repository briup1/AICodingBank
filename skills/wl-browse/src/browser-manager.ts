...
export class BrowserManager {
  ...
  async launch() {
    ...
    const standaloneExecutablePath = process.env.BROWSE_CHROMIUM_PATH || process.env.WL_BROWSE_CHROMIUM_PATH || undefined;
    this.browser = await chromium.launch({
      headless: useHeadless,
      ...(standaloneExecutablePath ? { executablePath: standaloneExecutablePath } : {}),
    ...
    this.browser.on('disconnected', () => {
      ...
      console.error('[browse] Console/network logs flushed to .wl-ai-skills/browse/browse-*.log');
      process.exit(1);
    });
  ...
  async launchHeaded(authToken?: string): Promise<void> {
    ...
    if (extensionPath) {
      ...
      // Write to ~/.local/state/wl-ai-skills/browse/.auth.json (not the extension dir, which may be read-only
      // in .app bundles and breaks codesigning).
      if (authToken) {
        const fs = require('fs');
        const path = require('path');
        const gstackDir = path.join(process.env.HOME || '/tmp', '.local', 'state', 'wl-ai-skills', 'browse');
    ...
    const fs = require('fs');
    const path = require('path');
    const userDataDir = path.join(process.env.HOME || '/tmp', '.local', 'state', 'wl-ai-skills', 'browse', 'chromium-profile');
    fs.mkdirSync(userDataDir, { recursive: true });

    // Support custom Chromium binary via WL_BROWSE_CHROMIUM_PATH env var.
    // Used by GStack Browser.app to point at the bundled Chromium.
    const executablePath = process.env.WL_BROWSE_CHROMIUM_PATH || undefined;
