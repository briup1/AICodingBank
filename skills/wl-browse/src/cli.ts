 ...
 *
 * Flow:
 *   1. Read .wl-ai-skills/browse/browse.json for port + token
...
function isNgrokAvailable(): boolean {
  ...
  const ngrokEnvPath = path.join(process.env.HOME || '/tmp', '.local', 'state', 'wl-ai-skills', 'browse', 'ngrok.env');
...
async function main() {
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    console.log(`wl-browse — Fast headless browser for AI coding agents
