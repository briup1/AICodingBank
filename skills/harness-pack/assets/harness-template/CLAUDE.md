...
> 你在本仓库的角色（职责与权限边界）见 `docs/harness/agents/owner.md`。
...
| 领域知识与契约 | `docs/harness/wiki/`（领域术语、业务模型、数据模型、接口协议） | {范围} | 人工 |

> 对本项目领域语义（名词、状态、数据结构、接口契约、能力流程）的任何“我以为”，先查 wiki 对应文档再动手，禁止凭猜测发明。
...
| 任务类型 | 读取入口 |
|----------|----------|
| 涉及领域概念、命名、状态、数据结构或接口调用/变更 | 先查 `wiki/` 对应文档（术语→领域术语；流程→业务模型；表结构与状态→数据模型；端点与契约→接口协议） |
