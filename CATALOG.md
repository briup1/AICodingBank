...
## Git
- **wl-git-requirement-flow**（自研·已验证 2026-09-21）— 从最新 master 安全创建和单向集成需求分支，并以用户级静态/localhost 实时 HTML 看板聚合多项目 Worktree、Git 生命周期与异常
  边界：看板只监控和刷新，不启动 Agent 或执行 Git 写操作；测试和上线结果需真实确认，远端分支仅在最终目标已包含当前 SHA 后删除
...
## 审查
- **wl-design-reachability-review**（自研·**待验证（未安装）**）— 从已实现的真实原点沿方案流程、状态和契约逐跳思维实验，证明需求终点是否可达
  边界：只发现设计 Gap、对接断点及不成立假设，不做泛化工程评审、不实现代码或扩大需求
- **wl-plan-design-review**（自研·已验证 2026-09-01）— 在编码前审查并完善 UI/UX 实施计划，使交互和视觉决策达到可实施状态
  边界：只审查前端体验与设计计划，不替代工程架构评审或代码实现
...
- **wl-design-doc**（自研·已验证 2026-08-31）— 方案设计——需求批准后产出可批准的《方案设计文档》（映射/契约/复用分析/多方案/附证据审查/最小验证）
  边界：前置 wl-req-confirm 已批准；只出方案不写实现代码，不替用户拍板选方案
- **wl-git-requirement-flow**（自研·已验证 2026-09-21）— 从最新 master 安全创建和单向集成需求分支，并以用户级静态/localhost 实时 HTML 看板聚合多项目 Worktree、Git 生命周期与异常
  边界：看板只监控和刷新，不启动 Agent 或执行 Git 写操作；测试和上线结果需真实确认，远端分支仅在最终目标已包含当前 SHA 后删除
...
- **wl-subagent-driven-development**（自研·已验证 2026-09-01）— 按已批准计划逐任务调度隔离 Worker，以测试优先、规格审查和质量审查完成开发
  边界：仅在方案已批准后使用；子 Agent 未获授权或不可用时必须在本地执行同等闸门
- **wl-ticket-run**（自研·已验证 2026-08-31）— 工单执行——方案批准后拆垂直切片工单（G/W/T 验收），/list 并行调度、/goal 审计执行到验收
  边界：前置 wl-design-doc 已批准；管执行纪律（台账/熔断/停机），不做需求与方案决策
